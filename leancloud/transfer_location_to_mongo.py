...
motion_query = Query(UserLocation)

client = MongoClient("mongodb://senzhub:Senz2everyone@119.254.111.40:27017")

count_query = Query(UserLocation)

db = client.RefinedLog
#print db.UserMotion.count()
import timeit
for times in range(count_query.count() / 500 + 1):

    motion_query.limit(500)
    motion_query.skip(times * 500)
    objects = motion_query.find()

    print len(objects)
    print "current time", timeit.default_timer()
    new_objs = []
    for object in objects:
        ...
        del object["user"]
        object["user_id"] = id
        if "location" in object.keys():
            lat = object["location"]["latitude"]
            lng = object["location"]["longitude"]
            object["location"] = dict(lat=lat,lng=lng)
        #print object

        #db.UserMotion.insert_one(object)
        new_objs.append(object)

    db.UserLocation.insert_many(new_objs)


a = {"location": {
    "lat": 39.941006000000001563,
    "lng": 116.35211499999999774
  }}
